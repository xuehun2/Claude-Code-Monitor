...
- **统计图表**：可按模型 / 时间范围 / 类型筛选的全功能图表，带 Y 轴刻度、悬停 tooltip 和汇总卡片
- 最近请求历史表
...
| --- | --- |
| 模型 | 全部 + 自动列出所有出现过的模型 |
| 时间范围 | 全部 / 最近1h / 6h / 24h / 7天 / 30天 / 自定义（起止时间） |
| 类型 | 输入token / 输出token / 速率 / 会话时间 |

图表展示筛选范围内的请求序列，Y 轴标注数值刻度，X 轴时间标签与柱子精确对齐，悬停柱状条显示具体数值与时间；上方汇总卡片显示数量、平均、最小、最大、合计。
