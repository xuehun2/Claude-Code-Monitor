...
针对"长会话流式输出时 Claude Code 页面卡在 thinking、会话结束才刷新"的问题继续优化。根因:所有 VS Code 扩展共享同一个扩展宿主线程,本插件在流式期间任何同步工作都会直接抢占 Claude Code 渲染所需的时间片。

- **限制单次日志解析量** - `checkFile` 之前会一次性读取并同步解析所有新增日志字节。若 Claude Code 在流式/工具执行期间日志爆发(可达数 MB),`buf.toString` + `split("\n")` + 逐行正则会长时间阻塞宿主线程。现在每次最多解析 512KB(只取最近一段、跳过中间),从源头杜绝大块同步解析
- **fs.watch 事件防抖** - Windows 上被频繁写入的日志文件会触发大量 `fs.watch` 事件,每个都进入事件循环。现在 200ms 防抖合并突发事件为一次检查,避免事件风暴抢占线程
- **会话文件解析缓存** - `listActiveSessions` 每 5s 扫描 `~/.claude/sessions/`,之前每次都重新读取+JSON.parse 所有会话文件(包括大量已死会话)。现在按文件路径+mtime 缓存解析结果,已死会话标记 `dead` 后永久跳过,避免重复读取解析
- **日志重扫跳过** - `findLatestLog` 每 10s 全量 stat 所有候选日志文件。现在当前日志文件仍活跃(5min 内有写入)时直接跳过重扫
- **新增 `enableLogTail` 配置** - 可一键关闭日志 tailer(CPU 最敏感部分),状态栏回退到纯 transcript 推断(无实时重试/首字节延迟信号)。若仍卡顿可先用此开关定位是否日志 tailer 所致,改后需重载窗口生效
